##############################################################################################################################################
#	Last Edited: 22-06-2018
#	blastNCBI_circos.R
#
#	SCRIPT THAT WILL QUERY GENES AGAINST NCBI (BlastP) TO ASSES THEIR PUTITATIVE ORIGIN	!!!!	
#	?! may serve as an idication for horizontal gene transfer events!	
#
#	putitative text for methods section
#	in order to determine the putitave origin of gene sets (for example unique genes present in a strain, or unique genes for a lineage), 
#	we perfom BLASTP searches of each gene against NCBI nr database (2018). we selected for all for all hits with e-value <10-5 and extracted
# 	the species that had the best hit. taxonomic hierarchy for these organisms were obtained via bacterio.net. we grouped the number of best hits at 
#	variable levels of taxonomic classification (genus, family, order, phylum). Chord diagrams were visualised using circilise in R.
#
# 	This script
#		1) query each gene against NCBI using blastp
#		5) store taxonomic information of blast hits
#		3) extract the best-hit
#		4) visualise the output with a chord diagram
#	
#	MAIN FUNCTIONS:
#		1) 	blastFASTA(InputFasta, GenusOfInterest, outdir)
#			designed to run over each protein sequence provided in a mutli-fasta file (AA sequences)
#			!This function is tailored to work independent of ProkComp			
#		
#		2)	blastANVIO(InputFasta, GenusOfInterest, outdir)
#			designed to run over each Protein Cluster (PC) sequence provided in vector of 'PC'-numbers (protein_cluster_id)'s
#			!This function is tailored to work as part of ProkComp
#
#	INPUT:
...
#		- pdfs with graphic representation of output data (Chord diagram)
#
#
##############################################################################################################################################
...
OverallTaxonData3 = OverallTaxonData2
OverallTaxonData3$query = sample(seq(from = 0, to = 50, by = 1), size = length(rownames(OverallTaxonData3)), replace = TRUE)
OverallTaxonData3$q2 = sample(seq(from = 0, to = 50, by = 1), size = length(rownames(OverallTaxonData3)), replace = TRUE)
OverallTaxonData3$q3 = sample(seq(from = 0, to = 50, by = 1), size = length(rownames(OverallTaxonData3)), replace = TRUE)
OverallTaxonData3$q4 = sample(seq(from = 0, to = 50, by = 1), size = length(rownames(OverallTaxonData3)), replace = TRUE)
OverallTaxonData3$q5 = sample(seq(from = 0, to = 50, by = 1), size = length(rownames(OverallTaxonData3)), replace = TRUE)
OverallTaxonData3$q6 = sample(seq(from = 0, to = 100, by = 1), size = length(rownames(OverallTaxonData3)), replace = TRUE)
OverallTaxonData3$q7 = sample(seq(from = 0, to = 50, by = 1), size = length(rownames(OverallTaxonData3)), replace = TRUE)

chordDiagram(t(OverallTaxonData3))
chordDiagram((OverallTaxonData3))

OverallTaxonData3 = OverallTaxonData2[1:3,]
OverallTaxonData3$query = sample(seq(from = 0, to = 50, by = 1), size = length(rownames(OverallTaxonData3)), replace = TRUE)
OverallTaxonData3$q2 = sample(seq(from = 0, to = 50, by = 1), size = length(rownames(OverallTaxonData3)), replace = TRUE)
OverallTaxonData3$q3 = sample(seq(from = 0, to = 50, by = 1), size = length(rownames(OverallTaxonData3)), replace = TRUE)
OverallTaxonData3$q4 = sample(seq(from = 0, to = 50, by = 1), size = length(rownames(OverallTaxonData3)), replace = TRUE)
